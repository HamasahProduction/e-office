...
@section('content')
    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="card">
                <form action="{{ route('app.admin.pengaturan.rt.update',[$rts->id]) }}" method="post" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-lg-9">
                                    <select required name="rw_id" id="rw_id"
                                        ...
                                        @foreach ($rws as $item)
                                            <option value="{{ $item->id }}" {{ $item->id==$rts->rw_id?'selected':'' }}>{{ $item->nomor }}</option>
                                        @endforeach
                                    </select>
                                    <div class="invalid-feedback">
                                        @error('rw_id')
                                ...
                                <div class="col-lg-9">
                                    <select required name="dusun_id" id="dusun_id"
                                        ...
                                        @foreach ($dusun as $item)
                                            <option value="{{ $item->id }}" {{ $item->id==$rts->dusun_id?'selected':'' }}>{{ $item->nama_dusun }}</option>
                                        @endforeach
                                    </select>
                                    <div class="invalid-feedback">
                                        @error('dusun_id')
