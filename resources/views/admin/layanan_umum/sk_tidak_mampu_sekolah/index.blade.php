...
@section('content')
    @component('components.breadcrumb')
        ...
        @endslot
        @slot('action_button')
            <a href="{{ route('app.admin.surat.sktm.create') }}" class="btn btn-primary btn-md">
                        ...
                        <div class="row filter-row">
                            ...
                            <div class="col-md-8 col-12">
                                <div class="col-md-8 float-start">
                                    <button type="submit"
                                        onclick="javascript: form.action='{{ route('app.admin.surat.sktm.index') }}';"
                        ...
                        <div class="tab-pane show active">
                            <div class="table-responsive">
                                <table class="table table-striped custom-table no-footer mb-0 datatable">
                                    ...
                                    <tbody>
                                        @foreach ($sktm as $pemohon)
                                            <tr>
                                                <td>{{ Carbon\Carbon::parse($pemohon->tgl_surat)->isoFormat('D MMMM Y') }}</td>
                                                ...
                                                <td>{!! wordwrap($pemohon->alamat, 40, "<br>\n") !!}</td>
                                                <td>Siswa/i {{ strtoupper($pemohon->kelas) }} <br>
                                                    {{ strtoupper($pemohon->nama_sekolah) }}
                                                </td>
                                                <td>
                                                    @if ($pemohon->is_cetak == true)
                                                        <button type="button"
                                                            data-action="{{ route('app.admin.surat.sktm.update.batalcetak', $pemohon->id) }}"
                                                            data-id="{{ $pemohon->id }}"
                                                    ...
                                                    @else
                                                        <button type="button"
                                                            data-action="{{ route('app.admin.surat.sktm.update.cetak', $pemohon->id) }}"
                                                            data-id="{{ $pemohon->id }}"
                                                ...
                                                <td>
                                                    ...
                                                    @else
                                                        <button type="button"
                                                            data-action="{{ route('app.admin.surat.sktm.response', $pemohon->id) }}"
                                                            data-id="{{ $pemohon->id }}"
                                                    ...
                                                    <div class="btn-group">
                                                        ...
                                                        <div class="dropdown-menu" style="">
                                                            <a class="dropdown-item btn-cetak-kades" href="#" data-id="{{ $pemohon->no_urut_surat }}"
                                                                data-url="{{ route('app.admin.surat.sktm.cetak') }}">Kepala Desa</a>
                                                            <a class="dropdown-item btn-cetak-an" href="#" data-id="{{ $pemohon->no_urut_surat }}"
                                                                data-url="{{ route('app.admin.surat.sktm.cetak') }}">Sekertaris Desa</a>
                                                        </div>
                                                    </div>
                                                    {{-- <button data-id="{{ $pemohon->no_urut_surat }}"
                                                        data-url="{{ route('app.admin.surat.sktm.cetak') }}"
                                                ...
                                                <td>
                                                    <a href="{{ route('app.admin.surat.sktm.edit', $pemohon->id) }}"
    ...
    <div class="row">
        <div class="col-lg-12" id="kepdes" style="display: none;">
            @include('admin.layanan_umum.sk_tidak_mampu_sekolah.template_surat.preview_surat')
        </div>
        <div class="col-lg-12" id="sekdes" style="display: none;">
            @include('admin.layanan_umum.sk_tidak_mampu_sekolah.template_surat.preview_surat_an')
        </div>
    </div>
@endsection
@push('scripts')
            ...
            $(".btn-cetak-kades").click(function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#status').text('Siswa/i Kelas ' + response.data.kelas + ' ' +
                            response.data.nama_sekolah);
            ...
            $(".btn-cetak-an").click(function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#status_an').text('Siswa/i Kelas ' + response.data.kelas + ' ' +
                            response.data.nama_sekolah);
