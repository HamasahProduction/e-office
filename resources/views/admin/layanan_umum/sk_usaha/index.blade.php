...
@section('content')
                        ...
                        <div class="row filter-row">
                            ...
                            <div class="col-md-8 col-12">
                                <div class="col-md-8 float-start">
                                    <button type="submit"
                                        onclick="javascript: form.action='{{ route('app.admin.surat.sku.index') }}';"
                        ...
                        <div class="tab-pane show active">
                            <div class="table-responsive">
                                <table class="table table-striped custom-table no-footer mb-0 datatable" id="datatable">
                                    ...
                                    <tbody>
                                        @foreach ($sk_usaha as $pemohon)
                                            <tr>
                                                <td>{{ Carbon\Carbon::parse($pemohon->tgl_surat)->isoFormat('D MMMM Y') }}
                                                </td>
                                                <td>{{ $pemohon->nomor_surat }}</td>
                                                <td>Nama : {{ $pemohon->nama_pemohon }} <br> NIK : {{ $pemohon->nik }} <br>
                                                    <small>Pekerjaan : {{ strtolower($pemohon->pekerjaan) }}</small>
                                                </td>

                                                <td>
                                                    <small>{{ $pemohon->penduduk->Rt->dusun }} <br>
                                                        {{ $pemohon->penduduk->Rt->nomor }} /
                                                        {{ $pemohon->penduduk->Rt->rw }}</small> <br>
                                                    {{-- {!! wordwrap($pemohon->alamat, 40, "<br>\n") !!} --}}
                                                </td>
                                                <td>{{ $pemohon->nama_usaha }}</td>

                                                <td>
                                                    @if ($pemohon->is_cetak == true)
                                                        <button type="button"
                                                            data-action="{{ route('app.admin.surat.sku.update.batalcetak', $pemohon->id) }}"
                                                            data-id="{{ $pemohon->id }}"
                                                    ...
                                                    @else
                                                        <button type="button"
                                                            data-action="{{ route('app.admin.surat.sku.update.cetak', $pemohon->id) }}"
                                                            data-id="{{ $pemohon->id }}"
                                                ...
                                                <td>
                                                    ...
                                                    @else
                                                        <button type="button"
                                                            data-action="{{ route('app.admin.surat.sku.response', $pemohon->id) }}"
                                                            data-id="{{ $pemohon->id }}"
                                                    ...
                                                    <div class="btn-group">
                                                        ...
                                                        <div class="dropdown-menu" style="">
                                                            <a class="dropdown-item btn-cetak-kades" href="#" data-id="{{ $pemohon->no_urut_surat }}"
                                                                data-url="{{ route('app.admin.surat.sku.cetak') }}">Kepala Desa</a>
                                                            <a class="dropdown-item btn-cetak-an" href="#" data-id="{{ $pemohon->no_urut_surat }}"
                                                                data-url="{{ route('app.admin.surat.sku.cetak') }}">Sekertaris Desa</a>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td>
                                                    <a href="{{ route('app.admin.surat.sku.edit', $pemohon->id) }}"
    ...
    <div class="row">
        <div class="col-lg-12" id="kepdes" style="display: none;">
            @include('admin.layanan_umum.sk_usaha.template_surat.preview_surat')
        </div>
        <div class="col-lg-12" id="sekdes" style="display: none;">
            @include('admin.layanan_umum.sk_usaha.template_surat.preview_surat_an')
        </div>
    </div>
@endsection
@push('style')
            ...
            $(".btn-cetak-kades").click(function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#nama_usaha').text(response.data.nama_usaha);
            ...
            $(".btn-cetak-an").click(function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#nama_usaha_an').text(response.data.nama_usaha);
                        $('#pekerjaan_an').text(response.data.pekerjaan);
