...
@section('content')
    @component('components.breadcrumb')
        ...
        @endslot
        @slot('action_button')
            <a href="{{ route('app.admin.surat.sktmr.create') }}" class="btn btn-primary btn-md">
                        ...
                        <div class="row filter-row">
                            ...
                            <div class="col-md-8 col-12">
                                <div class="col-md-8 float-start">
                                    <button type="submit"
                                        onclick="javascript: form.action='{{ route('app.admin.surat.sktmr.index') }}';"
                                    ...
                                    <tbody>
                                        @foreach ($sktmr as $pemohon)
                                            <tr>
                                                ...
                                                <td>{!! wordwrap($pemohon->penduduk->alamat, 50, "<br>\n") !!}</td>
                                                <td>
                                                    @if ($pemohon->is_cetak == true)
                                                        <button type="button"
                                                            data-action="{{ route('app.admin.surat.sktmr.update.batalcetak', $pemohon->id) }}"
                                                            data-id="{{ $pemohon->id }}"
                                                    ...
                                                    @else
                                                        <button type="button"
                                                            data-action="{{ route('app.admin.surat.sktmr.update.cetak', $pemohon->id) }}"
                                                            data-id="{{ $pemohon->id }}"
                                                ...
                                                <td>
                                                    ...
                                                    @else
                                                        <button type="button"
                                                            data-action="{{ route('app.admin.surat.sktmr.response', $pemohon->id) }}"
                                                            data-id="{{ $pemohon->id }}"
                                                    ...
                                                    <div class="btn-group">
                                                        ...
                                                        <div class="dropdown-menu" style="">
                                                            <a class="dropdown-item btn-cetak-kades" href="#" data-id="{{ $pemohon->no_urut_surat }}"
                                                                data-url="{{ route('app.admin.surat.sktmr.cetak') }}">Kepala Desa</a>
                                                            <a class="dropdown-item btn-cetak-an" href="#" data-id="{{ $pemohon->no_urut_surat }}"
                                                                data-url="{{ route('app.admin.surat.sktmr.cetak') }}">Sekertaris Desa</a>
                                                        </div>
                                                    </div>
                                                    {{-- <button data-id="{{ $pemohon->no_urut_surat }}"
                                                        data-url="{{ route('app.admin.surat.sktmr.cetak') }}"
                                                ...
                                                <td>
                                                    <a href="{{ route('app.admin.surat.sktmr.edit', $pemohon->id) }}"
    ...
    <div class="row">
        <div class="col-lg-12" id="kepdes" style="display: none;">
            @include('admin.layanan_umum.sk_tidak_memiliki_rumah.template_surat.preview_surat')
        </div>
        <div class="col-lg-12" id="sekdes" style="display: none;">
            @include('admin.layanan_umum.sk_tidak_memiliki_rumah.template_surat.preview_surat_an')
        </div>
    </div>
@endsection
@push('style')
            ...
            $(".btn-cetak-kades").click(function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#warganegara').text(response.warganegara);
